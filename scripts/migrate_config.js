...
async function run() {
    if (!connectionString || connectionString.includes('undefined')) {
        ...
        if (process.env.DATABASE_URL) console.log('Found DATABASE_URL');
        else console.log('Missing DATABASE_URL, attempting to construct from Supabase keys...');
        return;
    }
    ...
    try {
        ...
        const sqlPath = path.join(__dirname, '../supabase/migrations/013_create_ai_config.sql');
        const sql = fs.readFileSync(sqlPath, 'utf8');

        console.log('Running migration: 013_create_ai_config.sql');
        await client.query(sql);
        console.log('Migration successful! ai_config table created.');
